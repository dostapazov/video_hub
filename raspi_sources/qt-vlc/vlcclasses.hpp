...
{
  if(m_media)   libvlc_media_add_option(m_media,option) ;
}

inline void     vlc_media::add_option(const char * option,unsigned int flags)
{
    if(m_media) libvlc_media_add_option_flag(m_media,option,flags) ;
}
...
{
  if(m_media)   libvlc_media_set_user_data(m_media,data);
}


inline bool     vlc_player::is_playing()
{
  int p = libvlc_media_player_is_playing(m_player);
  return p ? true:false;
}
...
{
    if(m_player)
       {libvlc_media_player_play(m_player); return true;}
    return false;
}
...
{
   if(m_player)
      {libvlc_media_player_pause(m_player); return true;}
   return false;
}
...
{
  if(m_player) libvlc_set_fullscreen(m_player, full ? 1:0);
}
...
inline libvlc_state_t vlc_player::get_state ()
{
  ...
  {
     if(m_player)  libvlc_media_player_set_xwindow(m_player,x11drawable);
  }
