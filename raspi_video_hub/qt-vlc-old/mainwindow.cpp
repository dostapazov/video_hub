      ...
      case libvlc_MediaPlayerEncounteredError:
       {
           ...
           appLog::write(0,str);
           if(m_mon_player->has_media()) delete m_mon_player->set_media(nullptr);
       }
      break;
...
void MainWindow::on_moncam_timeout()
{
    ...
    if(m_mon_player)
    {
      ...
      {
        appLog::write(0,"monitor player is not playing ...");
        appLog::write(0,"delete current media and try restart");
        vlc::vlc_media * media  = m_mon_player->set_media(nullptr )  ;
        if(media) media->deleteLater();
...
void MainWindow::start_loggers()
{
    ...
    if(m_vlogger_state != vl_working)
    {
     ...
     if(0 == m_vlog_tmlen)  m_vlog_tmlen = 3600;
