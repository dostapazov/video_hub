...
static const quint8 auchCRCHi[] = {0x00, 0xC1, 0x81, 0x40, 0x01, 0xC0, 0x80, 0x41, 0x01, 0xC0,
...
static const quint8 auchCRCLo[] = { 0x00, 0xC0, 0xC1, 0x01, 0xC3, 0x03, 0x02, 0xC2, 0xC6, 0x06,
...
quint16 rtuCRC16(const char* data, quint16 len)
{
    ...
    for (int n = 0; n < len; n++)
    {
       i = crc_hi ^ quint8(*data++);
       crc_hi = crc_lo ^ auchCRCHi[i];
       crc_lo = auchCRCLo[i];
    }

    return quint16(crc_hi << 8) | crc_lo;
}
...
quint32 crc32(const QByteArray & data, quint64 start, quint64 len)
{
    ...
    int end = (len > 0)?  int(start+len) : data.count();
...
uint8_t crc8_1wire(const void *buf, int len, unsigned char clear)
{
    uint8_t crc;
    uint16_t cnt;
    ...
    for (cnt=0; cnt<len; cnt++)
        crc=crc8((reinterpret_cast<const uint8_t*>(buf))[cnt], crc);
